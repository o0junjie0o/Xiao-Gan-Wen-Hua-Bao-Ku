...
export default function AiStudio() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setResult(data);
      toast({ title: "✨ 创作完成！", description: "DeepSeek 已为您生成专属文创设计方案。" });
    } catch (err: unknown) {
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      <div className="container mx-auto px-4 py-16">
        ...
        >
          融合孝感非遗专属知识，由 DeepSeek 大模型驱动，一键生成独一无二的数字化文创设计方案。传统审美与现代 AI 的完美碰撞。
        </SectionHeading>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 max-w-6xl mx-auto">
          ...
          <div className="lg:col-span-5 space-y-7 bg-card p-6 md:p-8 rounded-3xl border border-border shadow-lg self-start">
            ...
            >
              {isGenerating ? (
                <>
                  <Sparkles className="w-5 h-5 animate-spin" /> DeepSeek 正在创作中...
                </>
              ) : (
                <>
            ...
            <div className="flex items-center gap-2 text-xs text-muted-foreground pt-1">
              ...
              <span>由 DeepSeek 大模型驱动 · 使用项目 DEEPSEEK_API_KEY</span>
            </div>
          </div>
            ...
            <AnimatePresence mode="wait">
              {isGenerating ? (
                ...
                >
                  ...
                  <h3 className="text-xl font-serif font-bold text-foreground">DeepSeek 正在注入灵感</h3>
                  ...
                  <div className="bg-card rounded-2xl border border-border overflow-hidden">
                    <div className="flex items-center justify-between px-6 py-4 border-b border-border">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-bold text-sm">DeepSeek 生成的设计方案</span>
                      </div>
                    ...
                    <div className="px-6 py-3 border-t border-border bg-muted/30 flex items-center justify-between">
                      <span className="text-xs text-muted-foreground">
                        {result.model ? `模型: ${result.model}` : "DeepSeek AI 生成"} · {new Date(result.generatedAt).toLocaleTimeString("zh-CN")}
                      </span>
                ...
                >
                  ...
                  <p className="text-sm text-muted-foreground mt-2 max-w-xs">
                    在左侧选择非遗风格、应用场景并输入创意描述，DeepSeek 将为您生成完整的文创设计方案
                  </p>
                </motion.div>
              )}
            </AnimatePresence>
