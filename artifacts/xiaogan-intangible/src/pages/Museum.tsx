...
export default function Museum() {
  ...
  const tabs = [
    ...
    { id: "national", label: "国家级", count: allItems.filter(i => i.level === "national").length },
    { id: "provincial", label: "省级", count: allItems.filter(i => i.level === "provincial").length },
    { id: "municipal", label: "市级", count: allItems.filter(i => i.level === "municipal").length },
  ];
      ...
      <div className="container mx-auto px-4 py-12">
              ...
              >
                ...
                {tab.count > 0 && (
                  <span className={clsx(
                    ...
                    activeLevel === tab.id ? "bg-white/20" : "bg-muted text-muted-foreground"
                  )}>
                    {tab.count}
                  </span>
                )}
        ...
        {!isLoading && (
          <p className="text-sm text-muted-foreground mb-6">
            共找到 <span className="text-primary font-semibold">{filteredItems.length}</span> 项非遗
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20 text-muted-foreground">
            ...
            <button onClick={() => { setSearchQuery(""); setActiveLevel("all"); }} className="mt-4 text-primary hover:underline text-sm">
              清除筛选条件
            </button>
          </div>
        ) : (
            ...
            {filteredItems.map((item, i) => (
              ...
              >
                ...
                <div className="p-5 flex-grow flex flex-col">
                  <div className="text-xs text-primary font-bold mb-1.5">{item.category}</div>
                  <h3 className="font-serif text-lg font-bold text-foreground mb-2 leading-snug">{item.name}</h3>
